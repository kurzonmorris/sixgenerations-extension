...
| `[?]` | **No answer given.** The returned file had no checkbox at all on these — sections 11–24 plus D-084 and D-106 lost their boxes somewhere between here and LibreOffice |
| *(note)* | Kurzon's own words, kept verbatim in meaning. These change the design and are carried into the docs |

**The `[?]` ones still need a yes or no.** Notes were written against D-209, D-233
and D-239, which are in that range — so the later sections were read, even though
nothing was ticked there. Several of them are also already implied by the written
answers at the bottom of this file. Fastest way to close it: say *"yes to
everything in 11–24"*, or name the exceptions.
